...
function wrap(body: string): string {
  ...
  <div style="max-width:560px;margin:0 auto;background:#fff;padding:28px;border:1px solid #e7e0d4">
    <p style="letter-spacing:.16em;text-transform:uppercase;font-size:11px;color:#0f5c4c;margin:0 0 16px">Client Kit</p>
    ${body}
  </div>
  </body></html>`;
}
...
export async function sendDocumentToClient(input: {
  ...
  await send(
    ...
    wrap(`
      ...
      <p><a href="${link}" style="display:inline-block;background:#0f5c4c;color:#fff;padding:12px 18px;text-decoration:none">Open document</a></p>
